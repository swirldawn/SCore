 ...
 *
  $upload=new Upload(array(
  				'rootPath'      =>  '/data/upload_file/', //保存根路径    可以自己定义路径和后面的依次组合构成完整的路径存放文件
...
class Upload{
    ...
    private $config = array(
        ...
        'subName'       =>  false, //子目录创建方式，[0]-函数名，[1]-参数，多个参数使用数组如果为false表示不设置子目录
        'rootPath'      =>  '', //保存根路径
    ...
    public function __construct($config=array()){
        ...
        if($this->config['rootPath']==""){
            $this->config['rootPath'] = config("application.upload_filepath_root");
        }
    }
    ...
    public function upload($file=''){
        ...
        if (!@move_uploaded_file($tempdir, $newDirName)) {//判断是否移动成功
            ...
            $retData['data']['file_size'] = Upload::formatSize($file['size']);
            $retData['data']['date'] = date('Y-n-d H:i:s',time());
            $retData['data']['type'] = $fileType['extension'];
            $retData['data']['md5'] = md5_file($newDirName);
    ...
    public function upBase64($base64Data)
    {
        ...
        if (!(file_put_contents($this->filePath, $img) && file_exists($this->filePath))) { //移动失败
            ...
            $retData['data']['file_size'] = Upload::formatSize($this->fileSize);
        }
        return $retData;
    }
    //检查大小
    ...
    private function setdir($type=false){
        ...
        if($this->config['pathTags']!=""){
            $dir=$dir.$this->config['pathTags'].'/';  //构造存储文件路径
        }

        if($subName!=false){
            $dir=$dir.$subName.'/';  //构造存储文件路径
        }
    ...
    public static function saveFile($file,$fileType,$path,$rootPath='/data/upload_file/'){
        ...
        if (!(file_put_contents($newDir.$fileName, $file) && file_exists($newDir.$fileName))) { //移动失败
            ...
            $retData['data']['file_size'] = Upload::formatSize(strlen($file));
            if($fileType=='amr'){
